...
#define PLATE_THUMBNAIL_WIDTH 300
#define PLATE_THUMBNAIL_HEIGHT 300
...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in Creality Print before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in Creality Print before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Creality Print before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in Creality Print before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in Creality Print before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in Creality Print before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Creality Print before uploading."},
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on Creality Print before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Creality Print."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    /*#if !defined(_WIN32) && !defined(__APPLE__)
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch CrealityPrint GUI soon";
        //
                    ...
                    for (int index = 0; index < partplate_list.get_plate_count(); index ++)
                    {
                        ...
                        if (print->empty()) {
                            ...
                            try {
                                ...
                                (dynamic_cast<Print*>(print))->set_is_BBL_printer(is_bbl_vendor_preset);
                        ...
                        for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                            ...
                            else if (!plate_data->thumbnail_file.empty() && (boost::filesystem::exists(plate_data->thumbnail_file)))
                            {
                                ...
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    ...
                                    unsigned int thumbnail_width = PLATE_THUMBNAIL_WIDTH, thumbnail_height = PLATE_THUMBNAIL_HEIGHT;
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into CrealityPrint process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a CrealityPrint process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the CrealityPrint process:\n\n";
        ...
        text += L"\n\n"
                ...
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes CrealityPrint "
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: CrealityPrint [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
